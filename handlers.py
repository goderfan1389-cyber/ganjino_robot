...
def process_message(msg):
    try:
        ...
        # هندل زندان با دکمه‌های شیشه‌ای
        ...
        if stripped == "/start":
            ...
            if now - u['last_claim'] < CLAIM_COOLDOWN:
                ...
                send_message(chat_id, f"*🔴 شما قبلاً طلا دریافت کردید 🪙\n\n🛠 لطفا {m} دقیقه و {s} ثانیه دیگر امتحان کنید.⚒*", parse_mode="Markdown", reply_to_message_id=reply_id); release_conn(conn); return
            ...
            if now - u['last_steal'] < STEAL_COOLDOWN:
                ...
                if warnings >= STEAL_WARNINGS_LIMIT:
                    ...
                    keyboard = {"inline_keyboard": [[{"text": f"💰 پرداخت فدیه ({JAIL_RANSOM} طلا)", "callback_data": f"jail_pay_{user_id}"}], [{"text": "⏳️تحمل می کنم", "callback_data": f"jail_wait_{user_id}"}]]}
    ...
    try:
        ...
        if data.startswith("jail_pay_"):
            ...
            if u['gold'] >= JAIL_RANSOM:
                ...
                edit_message(chat_id, msg_id, f"✅ شما با پرداخت {JAIL_RANSOM} طلا آزاد شدید!")
            else:
