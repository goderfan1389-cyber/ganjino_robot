...
admin_states = {} # برای مدیریت مراحل قرعه کشی
...
def handle_admin_commands(msg, u, conn, reply_id=None):
    ...
    if state:
        ...
        if state['step'] == 'time':
            try:
                ...
                # فوروارد پیام ادمین به گروه
                requests.post(f"{BASE_URL}/forwardMessage", data={
                    "chat_id": f"@{MAIN_GROUP_USERNAME}",
                    "from_chat_id": chat_id,
                    "message_id": state['data']['msg_id']
                })
        ...
        # --- مراحل پایان مسابقه و تعیین برنده ---
    ...
    if text == "همگانی":
        ...
        send_message(chat_id, "📤 پیام خود را بفرستید تا در گروه GANJINO_GAP ارسال شود. (برای لغو: انصراف)")
        return True
        
    if state and state['step'] == 'bc_msg':
    ...
    if text == "بکاپ":
        ...
        for row in rows:
            values = []
            for val in row:
                if isinstance(val, dict): val = json.dumps(val) # برای فیلد items
                if val is None: values.append("NULL")
                elif isinstance(val, (int, float)): values.append(str(val))
                else: values.append(f"'{str(val).replace(chr(39), chr(39)+chr(39))}'") # فرار از کوتیشن‌ها
